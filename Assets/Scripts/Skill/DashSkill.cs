...
public class DashSkill : MonoBehaviourPun, SkillMethod
{
    ...
    IEnumerator Attack(Vector3 desiredDir, float time)
    {
        ...
        // Two
        GameObject skill = PhotonNetwork.Instantiate("DodrugHitBox", transform.position, Quaternion.identity);//이펙트를 포톤 인스턴스를 합니다.
                                                                                                              // b.AddComponent<DodrugHit>();//이펙트에 히트 스크립트를 넣습니다.
        skill.SendMessage("AttackerName", gameObject.GetPhotonView().ViewID, SendMessageOptions.DontRequireReceiver);//이펙트에 공격자를 지정합니다.
        skill.transform.LookAt(desiredDir);
        skill.transform.Rotate(0, -180, 0);
